...
import gc
import sympy
from sympy import symbols, Or, And, Not, to_dnf, sympify
from sympy.logic.boolalg import BooleanTrue, BooleanFalse
from sympy.logic.boolalg import to_dnf, simplify_logic, BooleanTrue
import re
from z3 import *
...
followDataflows = True # Recommended: False.
...
addVariabilityInformation = False
addAssociatedComments = False
...
StatementTypes = ['FunctionDef', 'CompoundStatement', 'Statement', 'DeclStmt', 'StructUnionEnum', 'FunctionPointerDeclare', 'IfStatement', 'ElseStatement', 'SwitchStatement', 'ForStatement', 'DoStatement', 'WhileStatement', 'BreakStatement', 'ContinueStatement', 'GotoStatement', 'ReturnStatement', 'ExpressionStatement', 'IdentifierDeclStatement', 'PreIfStatement', 'PreElIfStatement', 'PreElseStatement', 'PreEndIfStatement', 'PreDefine', 'PreUndef', 'PreDiagnostic', 'PreOther', 'PreInclude', 'PreIncludeNext', 'PreLine']

def read_nodes_with_pc(file_path):
    """
    读取 nodesWithPC_new.csv 文件，并返回节点及其对应的 PC。
    :param file_path: CSV 文件路径
    :return: 字典，键为节点的 key，值为对应的 PC
    """
    nodes_df = pd.read_csv(file_path)

    # 拼接 'path' 和 'line' 列，形成新的键
    nodes_df['pathline'] = nodes_df['path'] + '-' + nodes_df['line'].astype(str)

    # 使用拼接后的 'key_combined' 列来创建字典
    nodes_dict = dict(zip(nodes_df['pathline'], nodes_df['PC'].str.strip()))
    # print("nodes_dict: ", nodes_dict, len(nodes_dict))
    return nodes_dict, nodes_df

def read_nodes_with_ID(file_path):
    nodes_df = pd.read_csv(file_path)
    # 拼接 'path' 和 'line' 列，形成新的键
    nodes_df['pathline'] = nodes_df['path'] + '-' + nodes_df['line'].astype(str)

    # 使用拼接后的 'key_combined' 列来创建字典
    nodes_dict = dict(zip(nodes_df['id'], nodes_df['pathline']))
    # print("nodes_dict: ", nodes_dict, len(nodes_dict))
    return nodes_dict, nodes_df
...
# FIs = []
# nodes_with_id_file = '/home/hining/codes/Jess/testProjects/libssh-0.5.3/newnodes.csv'
# nd, ndf = read_nodes_with_ID(nodes_with_id_file)
# check_statement_types = ['IdentifierDeclStatement', 'CallExpression']

def getRelPCNodes(passed_db, vertex_IDs, executor):
    global db, analysisList, checkedVertices, semanticUnit, executor_gl
    db = passed_db
    executor_gl = executor
    analysisList = list()
    checkedVertices = set()
    semanticUnit = set()
    return identifyRelPCNodes(vertex_IDs)

def initializeSUI_FIs(passed_db, vertex_IDs, pointType, nodes_dict, executor):
    global db, start_time, semanticUnit, FIUnit, preIfElifElse_node, checkedVertices, analysisList, externalFunctionsSet, externalMacrosSet, SUFilesSet, alreadyCheckedIdentifierDict, alreadyCheckedMacroIdentifierDict, executor_gl
    ...
    entryPointIds = vertex_IDs
    entryIdentifiers = list()
    entryFeatureNames = list()
    entryStrings = list()
    return identifySemanticUnits(entryPointIds, pointType, nodes_dict)
    # print("item: ", item)
    # if item:
    #     with open('/home/hining/codes/Jess/testProjects/libssh-0.5.3/output.txt', 'a') as file:
    #         # 将列表中的每个元素写入文件，每个元素占一行
    #         # for item in FIs:
    #         for expr_line in item:
    #             # 每条表达式写入一行，末尾自动加换行
    #             if expr_line != None and expr_line != 'False':
    #                 file.write(str(index) + " " + expr_line + "\n")
    #         del item
...
def initializeSUI(EVALUATION, entryPointType, pathOrNameOrIdentifierOrString, statementLine, statementType):
    global db, start_time, semanticUnit, FIUnit, preIfElifElse_node, checkedVertices, analysisList, externalFunctionsSet, externalMacrosSet, SUFilesSet, alreadyCheckedIdentifierDict, alreadyCheckedMacroIdentifierDict
    ...
    nodes_with_pc_file = '/home/hining/codes/Jess/testProjects/libssh-0.5.3/nodesWithPC.csv'

    # 读取节点信息及其对应的 PC
    nodes_dict, nodes_df = read_nodes_with_pc(nodes_with_pc_file)
    # workflowInput("Location", "src/options.c", "426", "ExpressionStatement")
    db.connectToDatabase("libssh053")

    # semanticUnit = set()
    # # save feature interactions of the semantic unit
    # FIUnit = {}
    # preIfElifElse_node = set()
    # # Initialize empty set of checked vertices (because we only need to check the vertices once)
    # checkedVertices = set()
    ...
    #     # get the ID of entrypoint
    #     workflowInput("Location", "src/ignore.c", "70", "ExpressionStatement")
    #
    #     # print("Project is set to: "+projectName)
    #     item = identifySemanticUnits(nodes_dict)



    for index, row in nodes_df.iterrows():
        if index >= 18244:
            if row['PC'] == 'True' or row['type'] not in check_statement_types:
                continue
            ...
            if (EVALUATION):
                ...
                if row['type'] == "CallExpression":
                    if workflowInput("Location", row['path'].split("/source/")[1], str(row['line']), 'ExpressionStatement') == 1:
                        continue
                else:
                    workflowInput("Location", row['path'].split("/source/")[1], str(row['line']), row['type'])
                ...
                query = "g.getGraph().clearCache()"
                executor_gl.submit(run_query, query)#db.runGremlinQuery(query)
                # gc.collect()
                time.sleep(5)
            if index % 20 == 0:
                executor_gl.submit(run_query, "quit")#db.runGremlinQuery("quit")
                db.connectToDatabase("libssh053")
                gc.collect()
...
def parse_node_expr(node_str: str):
    """
    将节点字符串解析为Sympy的逻辑表达式。
    假设原始表达式可能包含:
    - '!' 表示逻辑非，对应 sympy '~'
    - '&&' 表示逻辑与，对应 sympy '&'
    - '||' 表示逻辑或，对应 sympy '|'
    - 单独的变量名（字母、数字、下划线组成）
    需要将这些都替换为 sympy 可解析的表达式。
    """
    if not node_str:
        return BooleanTrue

    # 1. 替换 '&&'、'||'
    expr_str = node_str.replace('&&', ' & ').replace('||', ' | ')

    # 2. 替换 '!' 为 '~'
    expr_str = expr_str.replace('!', '~')
    expr_str = expr_str.replace(" 0", " False")
    # 如果变量名周围有多余的括号，例如 ~(HAVE_VASPRINTF)，这样是合法的：
    # ~HAVE_VASPRINTF 等价于 ~(HAVE_VASPRINTF)，sympy都能解析
    # 但如果您想严谨，可以尝试去掉单独包裹变量的括号
    # 可用正则替换单独包裹变量的括号。例如 (HAVE_VASPRINTF) -> HAVE_VASPRINTF
    ...
    sym_dict = {}
    for t in tokens:
        sym_dict[t] = symbols(t)

    # 调试输出
    # print("expr_str before sympify:", expr_str)
    ...
    try:
        ...
        print("Error parsing expression:", expr_str)
        raise e

    return expr

def extract_and_clauses_sympy(expr):
    # 将表达式转换为 DNF，force=True 可以在变量较多时强制转换
    dnf_expr = to_dnf(expr, simplify=False, force=True)

    # 根据 DNF 特性，dnf_expr 要么是单个子句（与关系），要么是或关系的合取子句
    if isinstance(dnf_expr, Or):
        clauses = dnf_expr.args  # 多个与子句通过或连接
    else:
        # 单个子句
        clauses = [dnf_expr]
    ...
    for clause in clauses:
        if clause == BooleanFalse:
            # 整个子句为False，可跳过
            continue
        if clause == BooleanTrue:
            # 整个子句为True，则没有约束可跳过
            continue
        ...
        for t in terms:
            if t == BooleanTrue:
                ...
                continue
            if t == BooleanFalse:
                # False会使整个子句无效，既然to_dnf后还保留False，理论上不该出现，但若有就跳过
                continue
            cleaned_terms.add(t)

        # 如果 cleaned_terms 为空，说明全部是True 或子句无效
        if not cleaned_terms:
            # 空子句等价于 True，不需要输出
            continue

        # 将 cleaned_terms 排序以保持一致性和可读性
        sorted_terms = sorted(cleaned_terms, key=lambda x: str(x))
        # 用 & 连接成字符串
        and_expr_str = ' & '.join(str(x) for x in sorted_terms)

        result.append(and_expr_str)

    return result


def get_PC_of_SUI(nodes_dict):
    # 构建 OR 集合
    or_expressions = []
    for verticeId, or_statement in FIUnit.items():
        or_terms = []
        for currentVerticeId in or_statement:
            pathLine = nd.get(currentVerticeId)
            node_str = nodes_dict.get(pathLine)
            if node_str is None:
                # 没有对应的特征表达式，默认当True处理或跳过？
                continue
            if isinstance(node_str, (int, float)):
                if math.isnan(node_str):
                    continue
            print("node_str: ", node_str)
            expr = parse_node_expr(node_str)
            or_terms.append(expr)
        if or_terms:
            # 多个term OR起来
            or_expr = Or(*or_terms)
            or_expressions.append(or_expr)

    # 构建 AND 集合
    and_terms = []
    for verticeId in semanticUnit:
        if verticeId in nd and verticeId not in preIfElifElse_node:
            pathLine = nd[verticeId]
            node_str = nodes_dict.get(pathLine)
            if node_str is None:
                continue
            if isinstance(node_str, (int, float)):
            ...
            print("node_str: ", node_str)
            expr = parse_node_expr(node_str)
            and_terms.append(expr)
        else:
            # 如果 verticeId 不在 nd 或在 preIfElifElse_node 中，跳过或根据需求处理
            pass

    if and_terms:
        current_PC_of_SUI = And(*and_terms)
    else:
        current_PC_of_SUI = True  # 没有条件则为 True

    for oe in or_expressions:
        current_PC_of_SUI = And(current_PC_of_SUI, oe)

    # 提取与关系表达式
    # print("current_PC_of_SUI: ", current_PC_of_SUI)
    and_clauses = extract_and_clauses_sympy(current_PC_of_SUI)
    # print("and_clauses: ", and_clauses)
    # # 打印结果
    # print("分解后的与关系表达式：")
    # for idx, clause in enumerate(and_clauses, 1):
    #     print(f"{idx}: {clause}")
    return and_clauses


def identifyRelPCNodes (entryPointIds):
    global db, executor_gl
    analysisList.extend(entryPointIds)
    while analysisList:
        node = analysisList.pop()
        # Check nodes only once
        if ((node not in checkedVertices) and node != "" and node not in check_skip_nodes):
            if node in key_relation_nodes:
                result = key_relation_nodes[node]
                ...
                # 5. Control Flow
                # 6. Variability
                analyzeNodeCallandDefRule(node)
            checkedVertices.add(node)
            semanticUnit.add(node)
    return semanticUnit

def identifySemanticUnits (entryPointIds, pointType, nodes_dict):
    global db, executor_gl
    ...
    print("--------------------------------------------------------------------------------- \n")
    #query = """g.V(%s).values('code')""" % (analysisList[0])


    # print("analysisList: ", analysisList, db.runGremlinQuery(query))
    ...
    print("semanticUnit: ", semanticUnit)
    if (len(semanticUnit) > 1):
        ...
        # get PC of SUI
        # return get_PC_of_SUI(nodes_dict)
        # if len(tem) != 0:
        #     return f"{tem}"
            # FIs.append(tem)
        # del tem
...
def analyzeNodeCallandDefRule(currentNode):
    ##################################################################################################################
    ################################### Call relations ###############################################################
    result = ""

    # Get type of current vertice
    query = """g.V(%s).valueMap('type', 'code')""" % (currentNode)
    type_and_code = executor_gl.submit(run_query, query).result()
    if (DEBUG): print("Check current node: " + str(currentNode) + " with type: " + str(type_and_code))

    # Inform the user if a node id does not exist
    if (len(type_and_code) < 1):
        print("################################################################################################")
        print("No vertice with the given id \"" + str(
            currentNode) + "\" found. Please check your database for an existing vertice id.")
        print("################################################################################################")
        del type_and_code
        return
    if 'code' not in type_and_code[0]:
        return
    type_of_current_node = type_and_code[0]['type'][0]
    if type_and_code[0]['code'][0] in ['NULL', 'True', 'False', '0', '1', 'sizeof']:
        check_skip_nodes.add(currentNode)
        if (DEBUG): print("Check NULL', 'True', 'False', '0', '1', 'sizeof")
        return

    ################################ No analysis ########################################################################################
    ################# No meaningful connections, no direct appearence in the code, already contained in other analyses ##################

    if (type_of_current_node in ['CompoundStatement', 'IncDec', 'UnaryOperator', 'AdditiveExpression',
                                 'PrimaryExpression', 'UnaryOperationExpression', 'ArrayIndexing',
                                 'RelationalExpression', 'Sizeof', 'SizeofOperand', 'ReturnType', 'CFGEntryNode',
                                 'CFGExitNode', 'PreMacroParameters', 'BreakStatement', 'ContinueStatement', 'Symbol',
                                 'IdentifierDeclType', 'IdentifierDecl', 'ParameterType', 'RelationalExpression',
                                 'Decl', 'DeclByType', 'StructUnionEnum', 'FunctionPointerDeclare', 'Label',
                                 'PreInclude', 'PreIncludeNext', 'Statement', 'Comment', 'CustomNode']):
        # There are no rules for these node types, so just skip them
        print("There are no rules for these node types, so just skip them")
        check_skip_nodes.add(currentNode)
        return
    # Get called function or function-like macro if current vertice is a callee
    if (type_of_current_node == "Callee"):
        # Do not look at CallExpression, one query is enough. Sometimes, we even do not have a CallExpression, just a Callee
        result = set(getCalledFunctionDefOrMacro(currentNode, type_of_current_node))
        # Get related elements of the called function
        key_relation_nodes[currentNode] = result
        analysisList.extend(result)
        # Print result
    ...
    if (type_of_current_node == "MacroCall"):
        result = set(getCalledMacroDef(currentNode, type_of_current_node))
        # Get related elements of the called function
        key_relation_nodes[currentNode] = result
        analysisList.extend(result)
        # Print result
    ...
    if (type_of_current_node in ('PreIfCondition', 'PreMacro')):
        result = set(getCalledFunctionDefOrMacro(currentNode, type_of_current_node))
    ...
    if ((type_of_current_node == "FunctionDef") and (lookForAllFunctionCalls == True)):
    ...
    if (type_of_current_node in ["PreUndef", "PreDefine"]) and (lookForAllMacroUsages == True):
    ...
    if (type_of_current_node == "PreMacroIdentifier") and (lookForAllMacroUsages == True):
    ...
    if (type_of_current_node == "Function"):
    ...
    if (type_of_current_node == "FunctionDef"):
    ...
    if (type_of_current_node == "DeclStmt"):
    ...
    if (type_of_current_node in ('Condition', 'PreIfCondition', 'Parameter', 'ParameterList')):
    ...
    if (type_of_current_node == "AddressOfExpression"):
        result = set(getCalledFunctionDefOrMacro(currentNode, type_of_current_node))
        key_relation_nodes[currentNode] = result
...
def analyzeNode (currentNode, nodes_dict):
    global DEBUG
    ...
    # Get type of current vertice 
    query = """g.V(%s).valueMap('type', 'code')""" % (currentNode)
    type_and_code = executor_gl.submit(run_query, query).result() #db.runGremlinQuery(query)
...
################# No meaningful connections, no direct appearence in the code, already contained in other analyses ##################
    ...
    if (type_of_current_node in ['CompoundStatement','IncDec','UnaryOperator','AdditiveExpression','PrimaryExpression','UnaryOperationExpression', 'ArrayIndexing', 'RelationalExpression', 'Sizeof', 'SizeofOperand', 'ReturnType','CFGEntryNode','CFGExitNode','PreMacroParameters','BreakStatement', 'ContinueStatement','Symbol','IdentifierDeclType','IdentifierDecl','ParameterType','RelationalExpression','Decl', 'DeclByType', 'StructUnionEnum', 'FunctionPointerDeclare', 'Label', 'PreInclude', 'PreIncludeNext', 'Statement', 'Comment', 'CustomNode']):
        #There are no rules for these node types, so just skip them
        print("There are no rules for these node types, so just skip them")
        check_skip_nodes.add(currentNode)
        return
...
################################ Structural relations ###################################################################
    ...
    if (type_of_current_node == "Directory"):
    ...
    if ((type_of_current_node == "FunctionDef") and (includeEnclosedCode == True)):
    ...
    if (type_of_current_node in ["IfStatement","ForStatement","WhileStatement","DoStatement","SwitchStatement"]) and (includeEnclosedCode == True):
        result = set(getASTChildren(currentNode))
        # For each enclosed vertice, add to the Semantic Unit and get related elements
        key_relation_nodes[currentNode] = result
        analysisList.extend(result) 
    ...
    elif (type_of_current_node in ["IfStatement","ForStatement","WhileStatement"]):
        # Get corresponding else-statement only if the configuration is selected
        if ((type_of_current_node == "IfStatement") and (connectIfWithElse == True)):
    ...
    if (type_of_current_node == "ElseStatement"):
    ...
    if (type_of_current_node in ["ExpressionStatement", "Condition", "IdentifierDeclStatement", "PreDefine", "PreDiagnostic", "PreOther"]):
        result = set(getASTChildren(currentNode))
...
##################################### Data Flow ##################################################################   
    ...
    if (followDataflows == True and type_of_current_node in ["IdentifierDeclStatement", "ExpressionStatement", "ReturnStatement"]):#["ForInit", "IdentifierDeclStatement", "Parameter", "AssignmentExpression", "ExpressionStatement", "Argument", "ArgumentList", "Condition", "UnaryExpression", "ReturnStatement"]):
        if currentNode not in nodes_dict:
            return
        query_verify_is_common_codes = f"g.V('{currentNode}').values('code')"
        # Maybe some types are missing, needs further testing
        result = set(getDefinesAndUses(currentNode, nodes_dict))
        key_relation_nodes[currentNode] = result
        # Get related elements of the called function
        analysisList.extend(result)
        # Print result
...
##################################### Control Flow ###############################################################       
    ...
    if (type_of_current_node == "GotoStatement"):
...
#################################### Variability ###################################################################          
    ...
    if (type_of_current_node == "PreIfStatement"):
        #get variable statements
        variableStataments = getVariableStatements(currentNode)
        result = set(variableStataments)
        if currentNode not in FIUnit:
            FIUnit[currentNode] = set()
        preIfElifElse_node.add(currentNode)
        isFirst = True
        for node in result:
            if isFirst:
                # FIUnit[currentNode].add(currentNode)
                FIUnit[currentNode].add(node)
                isFirst = False
            preIfElifElse_node.add(node)
        del isFirst
        # FIUnit[currentNode].append(tem_set)
    ...
    if (type_of_current_node == "PreElIfStatement"):
        # Get variable statements
        result = set(getVariableStatements(currentNode))
        start_if_node = getPreIf(currentNode)
        print("start_if_node: ", start_if_node)
        isFirst = True
        if start_if_node[0] not in FIUnit:
            FIUnit[start_if_node[0]] = set()
        for node in result:
            if isFirst:
                FIUnit[start_if_node[0]].add(node)
                # FIUnit[start_if_node[0]].add(currentNode)
                isFirst = False
            preIfElifElse_node.add(node)
        preIfElifElse_node.add(currentNode)
        del isFirst
        # Get the starting #if
        result.update(set(start_if_node))
    ...
    if (type_of_current_node == "PreElseStatement"):
        # Get variable statements
        result = set(getVariableStatements(currentNode))
        start_if_node = getPreIf(currentNode)
        isFirst = True
    ...
    if (type_of_current_node == "PreEndIfStatement"):
...
def getIncludedFiles (verticeId):
    ...
    return executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def getFunctionDeclInHeader (verticeId):
    ...
    fName = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def getFunctionDefinCFile (verticeId):
    ...
    names = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def getLabels (verticeId):
    ...
    name = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def getASTChildren (verticeId):
   #  query = """g.V(%s)
   # .emit()
   # .repeat(__.out('IS_AST_PARENT'))
   # .unfold()
   # .filter(__.values('code').not(within('NULL', 'True', 'False', '0', '1', 'sizeof')))
   # .dedup()
   # .id()""" % (verticeId)#
...
def getCalledMacroDef (verticeId, type):
    ...
    calledMacros = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
    ...
    for call in calledMacros:
        ...
        parentFileId = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
        ...
        # Run the query         
        sameFileDef = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
    ...
    #TODO: Do not stop if declaration is variable!
        ...
        # If there is no macro definition in the current file, check included files
            ...
            for file in fileList:   
                ...
                # Run the query            
                declResult = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def getCalledFunctionDefOrMacro (verticeId, type):
    ...
    functionNames = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def searchIncludesRecursively (rootFileID, currentIncludeChain, fileList):
    ...
    includes = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
    ...
    for include in includes:   
        ...
        fileID = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def getCallsToFunction (verticeId):      
    ...
    functionName = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def conflict_dnf(pc1_expr, pc2_expr):
    """
    判断两个 DNF 表达式（Z3 表达式）是否冲突。
    如果它们在任何模型下都不可能同时为真，则返回 True。
    """
    solver = Solver()
    solver.add(And(pc1_expr, pc2_expr))  # 如果存在模型使得它们都为真，则不冲突
    result = solver.check()
    return result == unsat

def parse_condition(cond):
    """
    解析条件字符串，返回正向特征集合和负向特征集合。
    例如，"HAVE_LIBCRYPTO & ~HAVE_OPENSSL_BLOWFISH_H" 将返回
      positive = {"HAVE_LIBCRYPTO"}
      negative = {"HAVE_OPENSSL_BLOWFISH_H"}
    """
    cond = cond.strip()
    # 用 & 分割条件，各条件之间可能有空格
    ...
    for token in tokens:
        token = token.strip()
        if token.startswith("~"):
            feature = token[1:].strip()
            negative.add(feature)
        else:
            positive.add(token)
    return positive, negative

def conflict(pos1, neg1, pc2):
    """
    判断两个条件是否存在冲突：
      如果某个特征在一个条件中是正向（存在），而在另一个条件中是负向（取非），
      则认为这两个条件冲突。
    返回冲突的特征集合（如果为空则无冲突）。
    """
    # pos1, neg1 = parse_condition(pc1)
    pos2, neg2 = parse_condition(pc2)
    # 检查是否有特征在 PC1 为正向而在 PC2 为负向，或反之
    conflict_features = (pos1 & neg2) | (pos2 & neg1)
    return conflict_features

# Get all statements that are connected via used and defined relations       
def getDefinesAndUses (verticeId, verticeWithPC):
    ...
    results = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
    new_results = set()
    # pos1, neg1 = parse_condition(verticeWithPC[verticeId])
    for vid in results:
        if vid in verticeWithPC:
            #if conflict(pos1, neg1, verticeWithPC[vid]):
            if conflict_dnf(verticeWithPC[verticeId], verticeWithPC[vid]):
                continue
            else:
                new_results.add(vid)
    return new_results
...
def getRelationsToMacro (verticeId):
    ...
    tempResult = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def getNodeParents (nodes, type):
    ...
    for currentNode in nodes:   
        ...
        nodes = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
        #Add the result
...
def addParentFunctions (nodes):
    ...
    query = f"idListToNodes(%s).repeat(__.in('IS_AST_PARENT')).until(has('type', 'FunctionDef')).dedup().id()" % (nodes)
    ...
    result = set(executor_gl.submit(run_query, query).result())       #db.runGremlinQuery(query)
    ...
    for node in result:
        if node not in semanticUnit:
            ...
            returnResults.update(set(executor_gl.submit(run_query, query).result()))   #db.runGremlinQuery(query)
...
def addParentBlocks (nodes):
    ...
    result = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def addLocalBackwardSlice (nodes):
    ...
    for node in nodes:
        ...
        query = """g.V(%s)
            ...
            .id()""" % (node)
        controlFlow = set(executor_gl.submit(run_query, query).result()) #db.runGremlinQuery(query)
        ...
        if (len(controlFlow) > 0):        
            ...
            dataFlow = set(executor_gl.submit(run_query, query).result())    #db.runGremlinQuery(query)
...
def getVariableStatements (verticeId):
    query = """g.V(%s)
   .out('VARIABILITY')
   .has('type', within('FunctionDef', 'CompoundStatement', 'Statement', 'DeclStmt', 'StructUnionEnum', 'FunctionPointerDeclare', 'IfStatement', 'ElseStatement', 'SwitchStatement', 'ForStatement', 'DoStatement', 'WhileStatement', 'BreakStatement', 'ContinueStatement', 'GotoStatement', 'ReturnStatement', 'ExpressionStatement', 'IdentifierDeclStatement', 'PreIfStatement', 'PreElIfStatement', 'PreElseStatement', 'PreEndIfStatement', 'PreDefine', 'PreUndef', 'PreDiagnostic', 'PreOther', 'PreInclude', 'PreIncludeNext', 'PreLine'))
   .dedup()
   .id()""" % (verticeId)
    return executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def addFilesIncludedBySUFilesRecursively ():
    ...
    for file in fileList:
        ...
        fileIDs = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
######## Caution: This will only return all StructUnionEnums that have an indentifier, this means there are some missing that need an extra query! ######       
        ...
        aResult = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
#ToDo handle FunctionPointerDeclares    
    ...
    bResult = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
    ...
    cResult = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def addProbablyUsedDefines(): 
    ...
    for line in aResult: 
        if not line['name'].endswith(")"):
            ...
            hasMacroContent = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def countNodes():
    ...
    query = "g.V().count()"
    statResult = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def workflowInput(entryPointType, pathOrNameOrIdentifierOrString, statementLine, statementType):
    ...
    # Project name
    # db.connectToDatabase("libssh053_2")
    ...
    if (entryPointType == "Feature"):
        ...
        # query = """g.V().has('path', textContains('%s')).has('line', '%s').id()""" % (statementPath, statementLine)
        print("query: ", query)
        result = executor_gl.submit(run_query, query).result()#db.runGremlinQuery(query)
...
def codeOutput ():
    ...
    for verticeId in semanticUnit: 
        ...
        code.append(executor_gl.submit(run_query, query).result()) #db.runGremlinQuery(query)
